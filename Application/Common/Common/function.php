...
function get_rongcloud_token($uid){
    ...
    // 获取用户昵称和头像
    $user_data=M('member')->field('username,avatar')->getById($uid);
    // 用户不存在
    if (empty($user_data)) {
        return false;
    }
...
//        die;
        ...
        if(!$info){
            ...
            // 返回成功信息
            foreach($info as $file){
                $datas = trim($file['savepath'].$file['savename'],'.');
 ...
 */
function pdf($html= '<h1 style="color:#ff0000">hello word</h1>'){
    vendor('Tcpdf.tcpdf');
...
//file_put_contents("./wx_app_".date("Y-m-dHis").".txt",print_r($pay,true));
/**
 * 验证AppStore内付
...
//        }
        ...
        $result =  D('config')->where($w)->select();
        if($result){
            ...
            foreach ($data as $k=>$v){
                ...
                if(!isset($temp[$k])){
                    ...
                    if($v!=$temp[$k]){
                        $name['name'] =$k;
                        M('config')->where($name)->save($newArr);
                    }

                }
            }
            //更新后的数据库记录
            $newRes = D('config')->where($w)->select();
    ...
    foreach ($list as $k => $v) {
        if ($v['cat_pid'] == $pid) {
            ...
            if ($level == 0) {
                ...
                $v['cat_name'] = kongge($level) . '├─ ' . $v['cat_name'];
            }

            $arr[] = $v;
            $arr = array_merge($arr, unlimitedForLevel($list, $v['cat_id'], $level + 1));
        }
    }
    return $arr;
}
...
//物流信息
function orderTracesSubByJson($code){
    $kd = C("kuaidi");
    ...
    $datas['DataSign'] = encrypt($requestData, $kd['appkey']);

    $order_code =json_decode(curlPost($kd['url2'], $datas),true);
    $LogisticCode = $order_code['LogisticCode'];
    $ShipperCode = $order_code['Shippers'][0]['ShipperCode'];
    ...
    $datas['DataSign'] = encrypt($requestData, $kd['appkey']);
    $result = json_decode(curlPost($kd['url2'], $datas),true);

    $result['ShipperName']=$order_code['Shippers'][0]['ShipperName'];
    return $result;
}
//返回封装
function msg($name){
    $type =array('add'=>'添加','edit'=>'修改','del'=>'删除');
    $msg = $type[$name];
    return $msg;
}
//根据openid获取用户id
function get_user_id($openid){
    $user = M('member')->where("wx_openid = '" . $openid . "'")->field('id')->find();
    return $user['id'];
}
//根据用户id获取用户信息
function get_user($user_id){
    $user = M('member')->where("id = '" . $user_id . "'")->find();
    $user['avatar'] = $user['avatar']?$user['avatar']:'/Upload/avatar/user1.jpg';
    return $user;
}
//根据用户id获取用户地址列表
function get_user_address($user_id){
    $wh['user_id'] = $user_id;
    $wh['is_default'] = 1;
    $user = M('users_address')->where($wh)->find();
    return $user;
}
//获取购物车
function get_goods_cart($cart_id){
    $wh['id'] = array('in',$cart_id);
    $cart_list = M('GoodsCart')->where($wh)->select();
    return $cart_list;
}
function new_trade_log($uid,$buyid,$goods_id,$goods_name,$price,$num,$desc,$commission){
    $data['uid'] =$uid;
    $data['buyid'] =$buyid;
    ...
    $re = M('trade_log')->add($data);
    return $re;
}
function new_log($dingdanhao,$jiage,$title,$des,$bz,$payment_id){
...
function getopenid($code, $appid, $appsecret)
{
    ...
    $info = curlGet($url);//发送HTTPs请求并获取返回的数据，推荐使用curl
...
function get_ip($ip){
    ...
    $path = "/query";
    $method = "GET";
    $appcode = "0bfab1c022474ebdb4876f1fb67932df";
    $headers = array();
...
function MakeSign($params,$key){
    //签名步骤一：按字典序排序数组参数
    ksort($params);
    $string = ToUrlParams2($params);  //参数进行拼接key=value&k=v
    //签名步骤二：在string后加入KEY
...
//小程序模板消息发送
function send_template($openid,$template_id,$page,$formid,$data,$order_id){
    ...
    $template_data['form_id']=$formid;
    $template_data['data']=$data;
    $template_data['emphasis_keyword']=$openid;
    $template_data = json_encode($template_data);
    $result = curlPost($url,$template_data);
    $json = json_decode($result);
    $arr = get_object_vars($json);
    if($arr['errmsg'] == "ok"){
        $w['order_id'] = $order_id;
        $save['status'] = 1;
        M('message')->where($w)->save($save);
    }
    return $result;
}
//post请求
 ...
 * * @param string $is 操作方式
 * @return  bool
 */
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$order_sn = '',$is=1){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
    ...
    $rr = M('account_log')->add($account_log);
    if($rr){
        return true;
    }else{
        return false;
...
function wxLogin(){
    $wx = C("wx_config");
    $appId = $wx['APPID'];
    $redirect_uri = urlencode('http://'.$_SERVER['HTTP_HOST'].'/index.php/Home/Index/getUserInfo');
 ...
 * @return booler   返回ajax的json格式数据
 */
function ajax_upload2($path='file',$format='empty',$maxSize='52428800'){
    ini_set('max_execution_time', '0');
    // 去除两边的/
    $path=trim($path,'/');
    // 添加Upload根目录
    ...
    if(!empty($_FILES)){
        ...
        if(!$info){
            ...
            // 返回成功信息
            foreach($info as $file){
                $data=trim($file['savepath'].$file['savename'],'.');
                // p($data);
...
//百度鉴定
function baidu_check($value,$type){
    require_once 'Baidu/AipContentCensor.php';
    $baidu      = C("bd_config");
    $appid      = $baidu['APPID'];
    $apikey     = $baidu['APP_KEY'];
    $secretkey  =$baidu['SECRET_KEY'];
    $client = new \AipContentCensor($appid, $apikey, $secretkey);
    if($type == 1){
        $result = $client->imageCensorUserDefined($value);
    }else{
        $result = $client->textCensorUserDefined($value);
    }
    if($result['conclusion'] != "合规"){
        die(json_encode(array('code' => 0, 'data' => '', 'msg' =>'~☺~净化网络环境从你我做起~☺~')));
    }
}
