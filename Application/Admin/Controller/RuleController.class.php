...
//******************用户-用户组*******************
    ...
    public function admin_user_list(){
        ...
        $show       = $Page->show();
        $data = M('AuthGroupAccess authgroupaccess')
            ->field('users.id,users.username,users.phone,authgroup.title')
            ->join('tp_users users ON users.id = authgroupaccess.uid')
            ->join('tp_auth_group authgroup ON authgroup.id = authgroupaccess.group_id')
    ...
    public function edit_users(){
        if(IS_POST){
            ...
            if($result){
                // 操作成功
                $this->success('修改成功',U('Admin/Rule/edit_users',array('id'=>$uid)));
            }else{
                $this->error('操作失败');
            }
        }else{
  ...
  /**
     * 产品增改页面
     */
    public function ycyj(){
        $param = I('get.');

        $where['id'] = $param['id'];

        $data = M('ycyj')->where($where)->find();
    ...
    public function ycyj_save(){
        ...
        if($param['type']=='add'){
            unset($param['type']);
            $result = M('ycyj')->add($param);

        }
        //修改
        if($param['type']=='edit'){
            unset($param['type']);
            $result = M('ycyj')->where('id='.$param['id'])->save($param);
        }

        //删除
        if($param['type']=='del'){
            $result = M('ycyj')->where('id='.$param['id'])->delete();
        }

        if ($result) {
                $this->success($msg.'成功',U('Admin/Rule/ycyj_list'));
        }else{
            $this->error($msg.'失败');
        }
    }
