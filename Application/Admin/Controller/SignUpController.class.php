...
class SignUpController extends AdminBaseController{
	...
	public function lists(){
	    $parm = I();
        if($parm['team_name']){
            $w['team_name']       = array('like','%'.$parm['team_name'].'%');
        }
        if($parm['team_leader_name']){
            $w['team_leader_name']       = array('like','%'.$parm['team_leader_name'].'%');
        }
        if($parm['team_leader_mobile']){
            $w['team_leader_mobile']       = array('like','%'.$parm['team_leader_mobile'].'%');
        }
        $count      = M('SignUp')->where($w)->count();
        $Page       = new \Org\Nx\Page($count,10);
        $show       = $Page->show();
        $list = M('SignUp')->where($w)->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('data',$list);
        $this->assign('page',$show);
        $this->assign('team_name',$parm['team_name']);
        $this->assign('team_leader_name',$parm['team_leader_name']);
        $this->assign('team_leader_mobile',$parm['team_leader_mobile']);
        $this->display();
	}
	public function download(){
        $param = I();
        $result = M('SignUp')->where('id='.$param['id'])->find();
        $video_name = "";
        download($result['video_url'],$video_name);
    }
